...
async function fundUsingTransfer({ algod, addressToFund, funding, fundAmount, transactionParams, sendParams, note, kmd, }) {
    ...
    const from = funding.fundingSource ?? (await getDispenserAccount.getDispenserAccount(algod, kmd));
...
async function ensureFunded(funding, algod, kmd) {
    ...
    if (fundAmount !== null) {
        if ((await networkClient.isTestNet(algod)) && fundingSource instanceof types_dispenserClient.TestNetDispenserApiClient) {
